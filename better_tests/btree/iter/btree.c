...
void bst_postorder(bst_node_t *tree) {
  ...
  // naplnenie zasobniku lavou diagonalou
  bst_leftmost_postorder(tree, &node_stack, &visit_stack);

  bool from_left = false;

  while (!stack_bst_empty(&node_stack)) {
    tree = stack_bst_top(&node_stack);
    from_left = stack_bool_top(&visit_stack);
    stack_bool_pop(&visit_stack);

    if (from_left) {
      stack_bool_push(&visit_stack, false);
      bst_leftmost_postorder(tree->right, &node_stack, &visit_stack);
    } else {
